...
class TaskList : Node() {
    ...
    override fun setContentByLocalJSON(js: JSONObject?) {
        ...
        try {
            ...
            if (folder.getInt(NoteColumns.Companion.TYPE) == Notes.TYPE_FOLDER) {
                ...
                if (folder.getLong(NoteColumns.Companion.ID) == Notes.ID_ROOT_FOLDER.toLong()) setName(
                    GTaskStringUtils.MIUI_FOLDER_PREFFIX + GTaskStringUtils.FOLDER_DEFAULT
                )
                else if (folder.getLong(NoteColumns.Companion.ID) == Notes.ID_CALL_RECORD_FOLDER.toLong()) setName(
    ...
    override fun getLocalJSONFromContent(): JSONObject? {
        try {
            ...
            var folderName = getName()
            if (getName().startsWith(GTaskStringUtils.MIUI_FOLDER_PREFFIX)) folderName =
    ...
    fun addChildTask(task: Task?): Boolean {
        ...
        if (task != null && !childTaskList.contains(task)) {
            ...
            if (ret) {
                ...
                task.setPriorSibling(
                    if (childTaskList.isEmpty()) null else childTaskList
                        .get(childTaskList.size - 1)
                )
                task.setParent(this)
            }
        }
        ...
        if (index != -1) {
            ...
            if (ret) {
                ...
                if (index != childTaskList.size) {
                    childTaskList.get(index).setPriorSibling(
