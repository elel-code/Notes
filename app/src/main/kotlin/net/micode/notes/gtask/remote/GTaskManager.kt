...
class GTaskManager private constructor() {
    ...
    fun sync(context: Context, onProgress: (String?) -> Unit): Int {
        if (mSyncing) {
        ...
        try {
            val client = GTaskClient.instance
            client.resetUpdateArray()

            // login google task
            if (!mCancelled) {
                val activity = mActivity ?: throw NetworkFailureException("activity context is not set")
            ...
            onProgress(mContext.getString(R.string.sync_progress_init_list))
            initGTaskList()

            // do content sync work
            onProgress(mContext.getString(R.string.sync_progress_syncing))
            syncContent()
    ...
    private fun initGTaskList() {
        ...
        val client = GTaskClient.instance
        try {
            val jsTaskLists = client.taskLists
            ...
            for (i in 0..<jsTaskLists.length()) {
                ...
                if (name
                    ...
                    mMetaList = TaskList()
                    requireMetaList().setContentByRemoteJSON(`object`)
                    ...
                    for (j in 0..<jsMetas.length()) {
                        ...
                        if (metaData.isWorthSaving()) {
                            requireMetaList().addChildTask(metaData)
                            if (metaData.gid != null) {
                                mMetaHashMap.put(metaData.relatedGid, metaData)
                            }
                        }
                    }
                }
            ...
            if (mMetaList == null) {
                mMetaList = TaskList()
                requireMetaList().name =
                    GTaskStringUtils.MIUI_FOLDER_PREFFIX + GTaskStringUtils.FOLDER_META
                GTaskClient.instance.createTaskList(requireMetaList())
            }
    ...
    private fun syncContent() {
            ...
            if (c != null) {
                while (c.moveToNext()) {
                    ...
                    if (node != null) {
                        ...
                        if (c.getString(SqlNote.Companion.GTASK_ID_COLUMN).trim().isEmpty()) {
                            // local add
        ...
        if (!mCancelled) {
            GTaskClient.instance.commitUpdate()
            refreshLocalSyncId()
        }
    }
    ...
    private fun syncFolder() {
        ...
        try {
            ...
            if (c != null) {
                ...
                if (node != null) {
                    ...
                    if (node.name != GTaskStringUtils.MIUI_FOLDER_PREFFIX + GTaskStringUtils.FOLDER_DEFAULT) doContentSync(
            ...
            if (c != null) {
                if (c.moveToNext()) {
                    ...
                    if (node != null) {
                        ...
                        // necessary
                        if (node.name != (GTaskStringUtils.MIUI_FOLDER_PREFFIX
        ...
        if (!mCancelled) GTaskClient.instance.commitUpdate()
    }
    ...
    private fun doContentSync(syncType: Int, node: Node?, c: Cursor?) {
        ...
        val meta: MetaData?
        when (syncType) {
            Node.Companion.SYNC_ACTION_ADD_LOCAL -> addLocalNode(requireNode(syncType, node))
            Node.Companion.SYNC_ACTION_ADD_REMOTE -> addRemoteNode(node, requireCursor(syncType, c))
            Node.Companion.SYNC_ACTION_DEL_LOCAL -> {
                val cursor = requireCursor(syncType, c)
                meta = mMetaHashMap.get(cursor.getString(SqlNote.Companion.GTASK_ID_COLUMN))
                if (meta != null) {
                    GTaskClient.instance.deleteNode(meta)
                }
                mLocalDeleteIdMap.add(cursor.getLong(SqlNote.Companion.ID_COLUMN))
            }

            Node.Companion.SYNC_ACTION_DEL_REMOTE -> {
                val remoteNode = requireNode(syncType, node)
                meta = mMetaHashMap.get(remoteNode.gid)
                if (meta != null) {
                    GTaskClient.instance.deleteNode(meta)
                }
                GTaskClient.instance.deleteNode(remoteNode)
            }

            Node.Companion.SYNC_ACTION_UPDATE_LOCAL -> updateLocalNode(
                requireNode(syncType, node),
                requireCursor(syncType, c)
            )
            Node.Companion.SYNC_ACTION_UPDATE_REMOTE -> updateRemoteNode(
                requireNode(syncType, node),
                requireCursor(syncType, c)
            )
            Node.Companion.SYNC_ACTION_UPDATE_CONFLICT ->                 // merging both modifications maybe a good idea
                // right now just use local update simply
                updateRemoteNode(requireNode(syncType, node), requireCursor(syncType, c))
    ...
    @Throws(ActionFailureException::class)
    private fun requireMetaList(): TaskList {
        return mMetaList ?: throw ActionFailureException("meta task list is not initialized")
    }

    @Throws(ActionFailureException::class)
    private fun requireNode(syncType: Int, node: Node?): Node {
        return node ?: throw ActionFailureException("missing remote node for sync action $syncType")
    }

    @Throws(ActionFailureException::class)
    private fun requireCursor(syncType: Int, cursor: Cursor?): Cursor {
        return cursor ?: throw ActionFailureException("missing local cursor for sync action $syncType")
    }
    ...
    private fun addLocalNode(node: Node) {
        ...
        val sqlNote: SqlNote?
        if (node is TaskList) {
            if (node.name == GTaskStringUtils.MIUI_FOLDER_PREFFIX + GTaskStringUtils.FOLDER_DEFAULT) {
                sqlNote = SqlNote(mContext, Notes.ID_ROOT_FOLDER.toLong())
            } else if (node.name == GTaskStringUtils.MIUI_FOLDER_PREFFIX + GTaskStringUtils.FOLDER_CALL_NOTE) {
                sqlNote = SqlNote(mContext, Notes.ID_CALL_RECORD_FOLDER.toLong())
            } else {
                sqlNote = SqlNote(mContext)
                sqlNote.setContent(
                    node.getLocalJSONFromContent()
                        ?: throw ActionFailureException("cannot create local folder content")
                )
                sqlNote.parentId = Notes.ID_ROOT_FOLDER.toLong()
            }
        } else {
            sqlNote = SqlNote(mContext)
            val js = node.localJsonFromContent
                ?: throw ActionFailureException("cannot create local note content")
            try {
            ...
            val parentId = mGidToNid[(node as Task).parent?.gid]
            if (parentId == null) {
                Log.e(TAG, "cannot find task's parent id locally")
                throw ActionFailureException("cannot add local node")
            }
            sqlNote.parentId = parentId
        }

        // create the local node
        sqlNote.setGtaskId(node.gid)
        sqlNote.commit(false)

        // update gid-nid mapping
        mGidToNid[node.gid] = sqlNote.id
        mNidToGid[sqlNote.id] = node.gid

        // update meta
        updateRemoteMeta(node.gid, sqlNote)
    }
    ...
    private fun updateLocalNode(node: Node, c: Cursor) {
        ...
        sqlNote.setContent(
            node.getLocalJSONFromContent()
                ?: throw ActionFailureException("cannot update local node content")
        )

        val parentId = if (node is Task)
            mGidToNid[node.parent?.gid]
    ...
    private fun addRemoteNode(node: Node?, c: Cursor) {
        ...
        if (sqlNote.isNoteType) {
            val task = Task()
            task.setContentByLocalJSON(sqlNote.content)

            val parentGid = mNidToGid[sqlNote.parentId]
            if (parentGid == null) {
                Log.e(TAG, "cannot find task's parent tasklist")
                throw ActionFailureException("cannot add remote task")
            }
            val parentTaskList = mGTaskListHashMap[parentGid]
                ?: throw ActionFailureException("cannot find task's parent tasklist")
            parentTaskList.addChildTask(task)

            GTaskClient.instance.createTask(task)
            n = task as Node

            // add meta
            updateRemoteMeta(task.gid, sqlNote)
        } else {
            var tasklist: TaskList? = null

            // we need to skip folder if it has already existed
            var folderName: String? = GTaskStringUtils.MIUI_FOLDER_PREFFIX
            if (sqlNote.id == Notes.ID_ROOT_FOLDER.toLong()) folderName += GTaskStringUtils.FOLDER_DEFAULT
            else if (sqlNote.id == Notes.ID_CALL_RECORD_FOLDER.toLong()) folderName += GTaskStringUtils.FOLDER_CALL_NOTE
            else folderName += sqlNote.snippet
            ...
            if (tasklist == null) {
                tasklist = TaskList()
                tasklist.setContentByLocalJSON(sqlNote.content)
                GTaskClient.instance.createTaskList(tasklist)
                mGTaskListHashMap[tasklist.gid] = tasklist
            }
            n = tasklist as Node
        }

        // update local note
        sqlNote.setGtaskId(n.gid)
        sqlNote.commit(false)
        sqlNote.resetLocalModified()
        sqlNote.commit(true)

        // gid-id mapping
        mGidToNid[n.gid] = sqlNote.id
        mNidToGid[sqlNote.id] = n.gid
    }
    ...
    private fun updateRemoteNode(node: Node, c: Cursor) {
        ...
        // update remotely
        node.setContentByLocalJSON(sqlNote.content)
        GTaskClient.instance.addUpdateNode(node)
        ...
        if (sqlNote.isNoteType) {
            ...
            val preParentList = task.parent ?: throw ActionFailureException("cannot update remote task")

            val curParentGid = mNidToGid[sqlNote.parentId]
            if (curParentGid == null) {
                Log.e(TAG, "cannot find task's parent tasklist")
                throw ActionFailureException("cannot update remote task")
            }
            val curParentList = mGTaskListHashMap[curParentGid]
                ?: throw ActionFailureException("cannot update remote task")

            if (preParentList !== curParentList) {
                preParentList.removeChildTask(task)
                curParentList.addChildTask(task)
                GTaskClient.instance.moveTask(task, preParentList, curParentList)
            }
        }
    ...
    private fun updateRemoteMeta(gid: String?, sqlNote: SqlNote?) {
        if (sqlNote != null && sqlNote.isNoteType) {
            ...
            if (metaData != null) {
                metaData.setMeta(
                    gid,
                    sqlNote.content ?: throw ActionFailureException("missing local note meta content")
                )
                GTaskClient.instance.addUpdateNode(metaData)
            } else {
                metaData = MetaData()
                metaData.setMeta(
                    gid,
                ...
                GTaskClient.instance.createTask(metaData)
            }
        }
    }
            ...
            if (c != null) {
                while (c.moveToNext()) {
                    ...
                    if (node != null) {
                        ...
                        values.put(NoteColumns.Companion.SYNC_ID, node.lastModified)
    ...
    val syncAccount: String
        get() = GTaskClient.instance.syncAccount?.name.orEmpty()
    ...
    companion object {
        ...
        val instance: GTaskManager by lazy(LazyThreadSafetyMode.SYNCHRONIZED) { GTaskManager() }
    }
}
