...
class SqlData {
    ...
    var content: JSONObject?
        ...
        set(js) {
            val json = js ?: return
            val dataId =
                if (json.has(Notes.DataColumns.Companion.ID)) json.getLong(Notes.DataColumns.Companion.ID) else INVALID_ID.toLong()
            ...
            val dataMimeType =
                if (json.has(Notes.DataColumns.Companion.MIME_TYPE))
                    json.getString(Notes.DataColumns.Companion.MIME_TYPE)
                else
                    DataConstants.NOTE
            ...
            val dataContent =
                if (json.has(Notes.DataColumns.Companion.CONTENT)) json.getString(
            ...
            val dataContentData1 =
                if (json.has(Notes.DataColumns.Companion.DATA1)) json.getLong(Notes.DataColumns.Companion.DATA1) else 0
            ...
            val dataContentData3 =
                if (json.has(Notes.DataColumns.Companion.DATA3)) json.getString(
    ...
    fun commit(noteId: Long, validateVersion: Boolean, version: Long) {
        if (mIsCreate) {
            ...
            val newId = uri?.pathSegments?.getOrNull(1)?.toLongOrNull()
            if (newId == null) {
                Log.e(TAG, "Get note id error: invalid insert result")
    ...
    companion object {
        ...
        val PROJECTION_DATA: Array<String> = arrayOf(
