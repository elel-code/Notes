...
class Note {
    ...
    var textDataId: Long
        get() = mNoteData.textDataId
        set(id) {
            mNoteData.textDataId = id
        }

    var callDataId: Long
        get() = mNoteData.callDataId
        set(id) {
            mNoteData.callDataId = id
        }
        ...
        fun pushIntoContentResolver(context: Context, noteId: Long): Uri? {
            ...
            if (mTextDataValues.size() > 0) {
                ...
                if (mTextDataId == 0L) {
                    ...
                    val newTextDataId = uri?.pathSegments?.getOrNull(1)?.toLongOrNull()
                    if (newTextDataId == null) {
                        Log.e(DATA_TAG, "Insert new text data fail with noteId$noteId")
                        mTextDataValues.clear()
                        return null
                    }
                    textDataId = newTextDataId
                } else {
            ...
            if (mCallDataValues.size() > 0) {
                ...
                if (mCallDataId == 0L) {
                    ...
                    val newCallDataId = uri?.pathSegments?.getOrNull(1)?.toLongOrNull()
                    if (newCallDataId == null) {
                        Log.e(DATA_TAG, "Insert new call data fail with noteId$noteId")
                        mCallDataValues.clear()
                        return null
                    }
                    callDataId = newCallDataId
                } else {
                    builder = ContentProviderOperation.newUpdate(
                        ContentUris.withAppendedId(
    ...
    companion object {
        ...
        private const val DATA_TAG = "NoteData"
        ...
        fun getNewNoteId(context: Context, folderId: Long): Long {
            ...
            val noteId = uri?.pathSegments?.getOrNull(1)?.toLongOrNull() ?: run {
