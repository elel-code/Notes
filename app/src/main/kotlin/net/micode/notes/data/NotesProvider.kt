...
class NotesProvider : ContentProvider() {
    private lateinit var mDatabase: NotesRoomDatabase

    override fun onCreate(): Boolean {
        mDatabase = NotesRoomDatabase(checkNotNull(context))
        return true
    }
    ...
    override fun query(
        ...
        when (mMatcher.match(uri)) {
            URI_NOTE -> c = db.query(
                buildQuery(TABLE.Companion.NOTE, projection, selection, selectionArgs, sortOrder)
            )
            ...
            URI_NOTE_ITEM -> {
                ...
                c = db.query(
                    buildQuery(
                        ...
                        projection,
                        NoteColumns.Companion.ID + "=" + id + parseSelection(selection),
                        selectionArgs,
                        sortOrder
                    )
                )
            ...
            URI_DATA -> c = db.query(
                buildQuery(TABLE.Companion.DATA, projection, selection, selectionArgs, sortOrder)
            )
        ...
        c?.setNotificationUri(checkNotNull(context).contentResolver, uri)
        return c
    }

    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        val db = mDatabase.openHelper.writableDatabase
        ...
        when (mMatcher.match(uri)) {
            URI_NOTE -> {
                noteId = db.insert(TABLE.Companion.NOTE, SQLiteDatabase.CONFLICT_NONE, contentValues)
                insertedId = noteId
            }

            URI_DATA -> {
                if (contentValues.containsKey(Notes.DataColumns.Companion.NOTE_ID)) {
                    noteId = contentValues.getAsLong(Notes.DataColumns.Companion.NOTE_ID)
                } else {
                    Log.d(TAG, "Wrong data format without note id:$contentValues")
                }
                run {
                    dataId = db.insert(TABLE.Companion.DATA, SQLiteDatabase.CONFLICT_NONE, contentValues)
                    insertedId = dataId
                }
            }
        ...
        // Notify the note uri
        if (noteId > 0) {
            checkNotNull(context).contentResolver.notifyChange(
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<String>?): Int {
        ...
        when (mMatcher.match(uri)) {
            URI_NOTE -> {
                ...
                count = db.delete(TABLE.Companion.NOTE, selection, selectionArgs.toBindArgs())
            }
            ...
            URI_NOTE_ITEM -> {
                ...
                count = db.delete(
                    TABLE.Companion.NOTE,
                    NoteColumns.Companion.ID + "=" + id + parseSelection(selection),
                    selectionArgs.toBindArgs()
                )
            }

            URI_DATA -> {
                count = db.delete(TABLE.Companion.DATA, selection, selectionArgs.toBindArgs())
                deleteData = true
            }
        ...
        if (count > 0) {
            if (deleteData) {
                checkNotNull(context).contentResolver.notifyChange(Notes.CONTENT_NOTE_URI, null)
            }
            checkNotNull(context).contentResolver.notifyChange(uri, null)
        }
        return count
    }
    ...
    private fun parseSelection(selection: String?): String {
        return if (!selection.isNullOrEmpty()) " AND ($selection)" else ""
    }
    ...
    private fun increaseNoteVersion(id: Long, selection: String?, selectionArgs: Array<String>?) {
        ...
        if (!TextUtils.isEmpty(selection)) {
            var selectString = if (id > 0) parseSelection(selection) else selection.orEmpty()
            selectionArgs.orEmpty().forEach { args ->
                selectString = selectString.replaceFirst("\\?".toRegex(), args.orEmpty())
            }
    ...
    private fun buildQuery(
        ...
        var builder = SupportSQLiteQueryBuilder.builder(table)
        if (projection != null) {
            builder = builder.columns(projection)
        }
        if (!selection.isNullOrEmpty()) {
            builder = builder.selection(selection, selectionArgs.toBindArgs())
        }
        if (!sortOrder.isNullOrEmpty()) {
            builder = builder.orderBy(sortOrder)
        }
